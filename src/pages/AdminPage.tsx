...
interface RegistrationPayload {
  ...
  haqiba: boolean | string | null;
  istima: boolean | string | null;
  totalPoints: number;
  notes: string;
...
export default function AdminPage() {
  ...
  // الحضور تلقائي (+10) لأي تسجيل؛ وبعدها تُضاف نقاط البنود الثابتة.
  // للمسارات الخاصة (نورانية/تلاوة) تُحسب نقاط البند من التقييم.
  const preview = ATTENDANCE_POINTS + (usesFirstCircleSpecialTrack
    ? specialTrackPoints +
      regItems
  ...
  const buildRegistrationPayload = (): RegistrationPayload => {
    ...
    const hifzValue = valueOf("hifz");
    const murajaaValue = valueOf("murajaa");
    const hifzFilled = !!hifzItem && regItemHasData(hifzItem, hifzValue);
    const murajaaFilled = !!murajaaItem && regItemHasData(murajaaItem, murajaaValue);
    // بند نعم/لا: "نعم" إذا اختارها المعلم، وإلا "لا".
    const yesNoText = (id: string) => (valueOf(id).value === true ? "نعم" : "لا");
    ...
    return {
      ...
      teamName: getTeamName(selectedTeam),
      date: getRiyadhDateISO(),
      // الحضور تلقائي: أي تسجيل من المعلم يعني الطالب حاضراً (+10).
      hudur: true,
      hifzFrom: hifzFilled && !hifzValue.absent ? formatSurahPoint(hifzValue.range.from) : null,
      hifzTo: hifzFilled && !hifzValue.absent ? formatSurahPoint(hifzValue.range.to) : null,
      hifzScore: hifzFilled
        ? (hifzValue.absent ? "لم يسمع" : regItemRangeScore(hifzItem as RegItem, hifzValue))
        : null,
      muraFrom: murajaaFilled && !murajaaValue.absent ? formatSurahPoint(murajaaValue.range.from) : null,
      muraTo: murajaaFilled && !murajaaValue.absent ? formatSurahPoint(murajaaValue.range.to) : null,
      muraScore: murajaaFilled
        ? (murajaaValue.absent ? "لم يسمع" : regItemRangeScore(murajaaItem as RegItem, murajaaValue))
        : null,
      haqiba: yesNoText("haqiba"),
      istima: yesNoText("istima"),
      totalPoints,
      notes: "",
    };
  };
          ...
          <div>
            ...
            {hasRegistrationData && (
              ...
              >
                {(
                  [
                    // الحضور تلقائي لكل تسجيل — يظهر دائماً في الملخص.
                    [
                      `${ATTENDANCE_LABEL} (تلقائي)`,
                      `+${ATTENDANCE_POINTS}`,
                    ] as [string, string],
